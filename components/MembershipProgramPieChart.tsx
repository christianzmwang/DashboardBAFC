...
import { programLegendPalette } from "./MembershipProgramBreakdownChart";

interface MembershipProgramPieChartProps {
  breakdown?: MonthlyProgramBreakdown; // specific month breakdown
  legendKeys: string[]; // ordered legend keys
  title: string;
  height?: number;
  showTotalBelowTitle?: boolean; // if true, render total under title instead of inside pie center
  onSliceClick?: (info: { program: string; value: number; month: string }) => void;
}

// Pie chart to show composition of active members by program for a single month.
export const MembershipProgramPieChart: React.FC<MembershipProgramPieChartProps> = ({
  breakdown,
  legendKeys,
  title,
  height = 260,
  ...
  const { data, total, month } = useMemo(() => {
    if (!breakdown) return { data: [] as Array<{ key: string; value: number }>, total: 0, month: "" };
    const programs = breakdown.programs || {};
    const rows: Array<{ key: string; value: number }> = [];
    for (const k of legendKeys) {
      const v = programs[k] || 0;
      if (v > 0) rows.push({ key: k, value: v });
    }
    const total = rows.reduce((s, r) => s + r.value, 0);
    return { data: rows, total, month: breakdown.month };
  ...
  useEffect(() => {
    ...
    const color = d3
      ...
      .domain(legendKeys)
      .range(legendKeys.map((_, i) => programLegendPalette[i % programLegendPalette.length]));
    ...
    arcs
      ...
      .on("click", function (_, d) {
        ...
        onSliceClick({ program: d.data.key, value: d.data.value, month });
      });
  ...
  }, [data, legendKeys, total, height, showTotalBelowTitle, onSliceClick, month]);
            ...
            {legendKeys.map((k, i) => {
              ...
              return (
                <li
                  ...
                  onClick={() => {
                    if (onSliceClick && breakdown) onSliceClick({ program: k, value: row.value, month: breakdown.month });
                  }}
                  title="Filter by this program"
                >
                  <span
                    className="inline-block w-3 h-3 rounded-sm"
                    style={{ backgroundColor: programLegendPalette[i % programLegendPalette.length] }}
                  />
                  <span>
                    {k} {pct >= 3 ? `(${pct.toFixed(1)}%)` : ""}
                  </span>
                </li>
              );
            })}
...
export default MembershipProgramPieChart;
