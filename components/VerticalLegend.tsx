...
export const VerticalLegend: React.FC<VerticalLegendProps> = ({
  ...
  return (
    <div className={`h-16 overflow-y-auto border border-gray-200 bg-white ${className}`}>
      <ul
        className={
          columns > 1
            ? `grid ${columns === 2 ? 'grid-cols-2' : columns === 3 ? 'grid-cols-3' : 'grid-cols-4'} gap-x-4 gap-y-1 pr-2`
            : 'space-y-1 pr-2'
        }
      >
        {keys.map((k, i) => (
          <li key={k} className="flex items-center gap-2 text-xs text-gray-700 h-5">
            <span
